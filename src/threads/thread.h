...
struct thread
  {
    ...
    struct file* thread_file;                //current file descriptor being used by current process

    bool load_status;                  //holds the load status of the child process, true if loaded, false otherwise

    int child_tid;                     //tid of the child that this thread is waiting for


    struct dir *directory;              //The directory that is being operated on currently

    int64_t break_sleep_tick;             //time at which thread stops sleeping
...
//New functions
struct thread *get_tid_thread(tid_t gtid);
void initialize_attributes(struct thread *input);
